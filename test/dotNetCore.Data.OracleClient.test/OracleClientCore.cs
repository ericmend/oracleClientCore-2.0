...
namespace dotNetCore.Data.OracleClient.test
{
    ...
    public class OracleClientCore
    {
        ...
        private const string userID = "usrtst";
        private const string password = "usrtst";
        ...
        [Fact]
        private void ReadingLargeDataSets()
        {
            using (var connection = new OracleConnection(connectionString))
            {
                connection.InfoMessage += new OracleInfoMessageEventHandler(OnInfoMessage);
                ...
                try
                {
                    ...
                    selectCount.CommandText = "SELECT COUNT(1) FROM MONO_TEST_TABLE8";

                    decimal countInsert = (decimal)selectCount.ExecuteScalar();
                    createTable = (countInsert != maxRows);
                }
                catch
                {
                    // ignore if table already exists
                ...
                if (createTable)
                {
                    try
                    {
                        cmd2.CommandText = "DROP TABLE MONO_TEST_TABLE8";
                        cmd2.ExecuteNonQuery();
                    }
                    catch (OracleException)
                    {
                    ...
                    cmd2.CommandText = "CREATE TABLE MONO_TEST_TABLE8(" +
                        ...
                        " COL6 DATE NOT NULL, " +
                        " COL7 DATE, " +
                        " COL8 VARCHAR2(8) NOT NULL, " +
                        " COL9 VARCHAR2(32), " +
                        " COL10 NUMBER(18,2) NOT NULL, " +
                        " COL11 NUMBER(18,2), " +
                        " COL12 DATE NOT NULL, " +
                        " COL13 DATE, " +
                        " COL14 VARCHAR2(8) NOT NULL, " +
                        " COL15 VARCHAR2(32), " +
                        " COL16 NUMBER(18,2) NOT NULL, " +
                        " COL17 NUMBER(18,2), " +
                        " COL18 DATE NOT NULL, " +
                        " COL19 DATE, " +
                        " COL20 VARCHAR2(8) NOT NULL, " +
                        " COL21 VARCHAR2(32) " +
                        ")";
                    cmd2.ExecuteNonQuery();
                    ...
                    decimal d = 1234567890123456.78M;

                    for (int i = 0; i < maxRows; i++)
                    {
                    	OracleCommand cmd = connection.CreateCommand();

                    	cmd.CommandText = "INSERT INTO MONO_TEST_TABLE8 " +
                        ...
                        OracleParameter parm1 = cmd.Parameters.Add(":P1", OracleType.Number);

                    	OracleParameter parm2 = cmd.Parameters.Add(":P2", OracleType.VarChar, 8);
                    	OracleParameter parm3 = cmd.Parameters.Add(":P3", OracleType.VarChar, 32);

                    	OracleParameter parm4 = cmd.Parameters.Add(":P4", OracleType.Number);
                    	OracleParameter parm5 = cmd.Parameters.Add(":P5", OracleType.Number);

                    	OracleParameter parm6 = cmd.Parameters.Add(":P6", OracleType.DateTime);
                    	OracleParameter parm7 = cmd.Parameters.Add(":P7", OracleType.DateTime);

                    	OracleParameter parm8 = cmd.Parameters.Add(":P8", OracleType.VarChar, 8);
                    	OracleParameter parm9 = cmd.Parameters.Add(":P9", OracleType.VarChar, 32);

                    	OracleParameter parm10 = cmd.Parameters.Add(":P10", OracleType.Number);
                    	OracleParameter parm11 = cmd.Parameters.Add(":P11", OracleType.Number);

                    	OracleParameter parm12 = cmd.Parameters.Add(":P12", OracleType.DateTime);
                    	OracleParameter parm13 = cmd.Parameters.Add(":P13", OracleType.DateTime);

                        OracleParameter parm14 = cmd.Parameters.Add(":P14", OracleType.VarChar, 8);
                    	OracleParameter parm15 = cmd.Parameters.Add(":P15", OracleType.VarChar, 32);

                    	OracleParameter parm16 = cmd.Parameters.Add(":P16", OracleType.Number);
                    	OracleParameter parm17 = cmd.Parameters.Add(":P17", OracleType.Number);

                    	OracleParameter parm18 = cmd.Parameters.Add(":P18", OracleType.DateTime);
                    	OracleParameter parm19 = cmd.Parameters.Add(":P19", OracleType.DateTime);

                    	OracleParameter parm20 = cmd.Parameters.Add(":P20", OracleType.VarChar, 8);
                    	OracleParameter parm21 = cmd.Parameters.Add(":P21", OracleType.VarChar, 32);

                    	// TODO: implement out, return, and ref parameters
                        
                        parm1.Value = i;

                    	parm2.Value = GetRandomString(8);
                    	parm3.Value = DBNull.Value;

                    	parm4.Value = d;
                    	parm5.Value = DBNull.Value;

                    	parm6.Value = DateTime.Now;
                    	parm7.Value = DBNull.Value;

                    	parm8.Value = GetRandomString(8);
                    	parm9.Value = DBNull.Value;

                    	parm10.Value = d;
                    	parm11.Value = DBNull.Value;

                    	parm12.Value = DateTime.Now;
                    	parm13.Value = DBNull.Value;

                    	parm14.Value = GetRandomString(8);
                    	parm15.Value = DBNull.Value;

                    	parm16.Value = d;
                    	parm17.Value = DBNull.Value;

                    	parm18.Value = DateTime.Now;
                    	parm19.Value = DBNull.Value;

                    	parm20.Value = GetRandomString(8);
                    	parm21.Value = DBNull.Value;
                ...
                OracleCommand select = connection.CreateCommand();
                select.CommandText = "SELECT COL10,COL9,COL8,COL7,COL6,COL5,COL4,COL3,COL2,COL1," +
                                            "COL11,COL12,COL13,COL14,COL15,COL16,COL17,COL18,COL19,COL20,COL21 FROM MONO_TEST_TABLE8";

                
                using (OracleDataReader reader = select.ExecuteReader())
                {
                    int[] ordinal = new int[21];
                    for (int i = 0; i < ordinal.Length; i++)
                    {
                        ordinal[i] = reader.GetOrdinal("COL" + (i + 1).ToString());
                    }
                    ...
                    while (reader.Read())
                    {
                        ...
                        int i = 0;
                        if (reader.IsDBNull(ordinal[i])) { row[ordinal[i]] = DBNull.Value; } else { row[ordinal[i]] = reader.GetInt64(ordinal[i]); }  i++;
                        if (reader.IsDBNull(ordinal[i])) { row[ordinal[i]] = DBNull.Value; } else { row[ordinal[i]] = reader.GetString(ordinal[i]); }  i++;
                        if (reader.IsDBNull(ordinal[i])) { row[ordinal[i]] = DBNull.Value; } else { row[ordinal[i]] = reader.GetString(ordinal[i]); }  i++;
                        if (reader.IsDBNull(ordinal[i])) { row[ordinal[i]] = DBNull.Value; } else { row[ordinal[i]] = reader.GetDecimal(ordinal[i]); }  i++;
                        if (reader.IsDBNull(ordinal[i])) { row[ordinal[i]] = DBNull.Value; } else { row[ordinal[i]] = reader.GetDecimal(ordinal[i]); }  i++;
                        if (reader.IsDBNull(ordinal[i])) { row[ordinal[i]] = DBNull.Value; } else { row[ordinal[i]] = reader.GetDateTime(ordinal[i]); }  i++;
                        if (reader.IsDBNull(ordinal[i])) { row[ordinal[i]] = DBNull.Value; } else { row[ordinal[i]] = reader.GetDateTime(ordinal[i]); }  i++;
                        if (reader.IsDBNull(ordinal[i])) { row[ordinal[i]] = DBNull.Value; } else { row[ordinal[i]] = reader.GetString(ordinal[i]); }  i++;
                        if (reader.IsDBNull(ordinal[i])) { row[ordinal[i]] = DBNull.Value; } else { row[ordinal[i]] = reader.GetString(ordinal[i]); }  i++;
                        if (reader.IsDBNull(ordinal[i])) { row[ordinal[i]] = DBNull.Value; } else { row[ordinal[i]] = reader.GetDecimal(ordinal[i]); }  i++;
                        ...
                        if (reader.IsDBNull(ordinal[i])) { row[ordinal[i]] = DBNull.Value; } else { row[ordinal[i]] = reader.GetDateTime(ordinal[i]); }  i++;
                        if (reader.IsDBNull(ordinal[i])) { row[ordinal[i]] = DBNull.Value; } else { row[ordinal[i]] = reader.GetString(ordinal[i]); }  i++;
                        if (reader.IsDBNull(ordinal[i])) { row[ordinal[i]] = DBNull.Value; } else { row[ordinal[i]] = reader.GetString(ordinal[i]); }
                        col.Add(row);
                    }
                    stopWatch.Stop();
